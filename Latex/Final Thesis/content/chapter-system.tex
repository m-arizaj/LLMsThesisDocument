\chapter{Systematic Mapping and Taxonomy Development}
\label{chapter3}
...
Through this search and filtering process, a total of 68 primary sources were selected as foundational to the construction of the LLMs Metrics Catalog. These references are documented and accessible on the bibliography (\ref{sec:bibliography}) section of this thesis, along with additional references for every metric.
...
\begin{enumerate}
	...
	\item Frequency Calculation: After the normalization process, we use Pandas to perform a frequency calculation that consisted of counting the times a metric appeared in the literature. This analysis helped us identify the most prevalent metrics in the community.
...
Metric Clusters: We define a "cluster" as a specific evaluation method that joins all its naming variants and specific implementations. For example, pass@1, pass@10, and pass@k were grouped into a single cluster named Pass@k. This process allowed us to reduce the initial 300 into 128 distinct metric clusters.

Taxonomy Categories: These represent high-level quality dimensions. Each category groups multiple Metric Clusters that share similar characteristics or goals. To make an example, all clusters relying on computational efficiency are grouped under Efficiency \& Resource Usage.
...
During the previous steps, we identified metrics within the context of Software Engineering based on literature regarding LLM evaluation. While these papers served as the foundation of our project, relying solely on broad sources can lead to ambiguity, as definitions are often tailored to specific experimental contexts. Consequently, to ensure the accuracy of this catalog, an additional reference validation step was performed.

This procedure was applied to any metric where the primary source provided an incomplete definition or omitted the mathematical formula. In such instances, we traced citations back to the seminal work—the original publication where the metric was first proposed—to ensure foundational accuracy.

This enrichment phase enabled us to:

\begin{enumerate}
    \item \textbf{Cross-Verification:} We contrasted the definitions found in survey papers against the original literature to determine if the concepts remained consistent or were modified for specific experimental contexts.
    \item \textbf{Content Enrichment:} By consulting seminal works, we recovered precise theoretical foundations, mathematical formalizations, and intended usages that are frequently omitted in broader surveys.
    \item \textbf{Reliability Assurance:} We provide canonical definitions, enabling users to rely on the thesis as an accurate source while retaining the ability to verify details against the original citations.
    \item \textbf{Traceability:} We established direct links between each metric and its primary source, facilitating access to the foundational research for deeper investigation.
\end{enumerate}
