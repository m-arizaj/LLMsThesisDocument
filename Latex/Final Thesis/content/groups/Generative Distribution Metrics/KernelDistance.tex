...
\subsubsection{Applications in Model Engineering}

While primarily established as an evaluation metric for image generation, the Kernel Distance (specifically KID) has been applied to automated software engineering tasks within the machine learning development lifecycle, particularly in training stability and hyperparameter optimization.

\begin{itemize}
    \item \textbf{Automated Learning Rate Adaptation}: In the engineering of GAN training pipelines, selecting learning rate schedules usually requires manual tuning. Bińkowski et al. (2018) propose using KID as a real-time monitoring signal to automate this process. By performing a relative similarity test on KID scores between iterations, the training loop can statistically detect convergence stagnation and trigger an automatic decay of the learning rate, removing the need for pre-defined schedules \cite{binkowski2018demystifying}.

    \item \textbf{Benchmarking and Regression Testing}: For the iterative development of generative software, reliable metrics are required to validate new model versions (regression testing). Stein et al. (2023) highlight that while traditional metrics like FID can misalign with human perception (penalizing diffusion models unfairly), KD/KID—especially when paired with modern encoders like DINOv2—provides a more robust distance measure for ranking disparate model architectures (e.g., Diffusion vs. GANs). This robustness makes it suitable for automated benchmarking pipelines where "ground truth" human evaluation is not feasible for every iteration \cite{Stein2023MetricFlaws}.
\end{itemize}
