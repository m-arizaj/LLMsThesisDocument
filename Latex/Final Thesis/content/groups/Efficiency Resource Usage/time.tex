...
\begin{itemize}
    ...
    \begin{equation} \bar{T}_{\text{norm}} = \frac{1}{n} \sum_i \frac{T_i}{T_{\text{baseline}}} \end{equation}

    \item \textbf{Timeout Ratio:}
    Proportion of tasks exceeding a predefined time threshold.
    \begin{equation} \text{TimeoutRatio} = \frac{N_{\text{timeouts}}}{N_{\text{total}}} \end{equation}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{equation} \alpha_k = \left(1 - \frac{|t_{lm} - t_{GT}|}{t_{GT}}\right) \times 100\% \end{equation}
    \item \textbf{Scaled Error Rate ($\epsilon^*_k$):}
    \begin{equation} \epsilon^*_k = \frac{10^3 \Delta_k [\mu s]}{t_{GT} [ms]} \end{equation}
\end{itemize}
