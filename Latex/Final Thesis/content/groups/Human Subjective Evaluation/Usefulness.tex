...
The Usefulness metric is applied in Software Engineering research to validate automated metrics and evaluate LLM-based coding assistants.

\begin{itemize}
    \item \textbf{Benchmarking Code Generators}: It serves as the "gold standard" for evaluating models on datasets like CoNaLa (Stack Overflow snippets), distinguishing between models that produce syntactically correct but useless code and those that provide practically helpful solutions \cite{yin2018learning, Evtikhiev2023OutOfBLEU}.
    \item \textbf{Calibration of Automated Metrics}: The metric is used to measure the correlation of automated metrics (e.g., BLEU, CodeBLEU, RUBY) with human preference. Studies have shown that traditional NLP metrics often have weak correlation with the Usefulness Score, necessitating new metrics like ICE-Score \cite{Zhuo2023ICEScore}.
    \item \textbf{LLM-as-a-Judge}: In modern applications, Large Language Models are instructed to simulate this human evaluation (e.g., the ICE-Score framework), predicting the Usefulness Score of a snippet to provide scalable, reference-free evaluation for code intelligence tasks \cite{Zhuo2023ICEScore}.
\end{itemize}
