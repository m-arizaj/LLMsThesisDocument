...
While traditional benchmarks in Software Engineering (such as SWE-bench) rely on objective metrics like issue resolution rates, the Subjective Score is applied to the \textbf{Computer Science} domain to evaluate the quality of Human-AI collaboration \cite{Guo2025HumanCentricEvaluation}.

\begin{itemize}
    \item \textbf{Innovation-Driven Problem Solving}: Within Computer Science, the metric is used to evaluate a model's capability to propose novel hypotheses or solutions for research gaps, moving beyond the static code generation tasks found in objective datasets \cite{Guo2025HumanCentricEvaluation}.
    \item \textbf{Assessment of Technical Interaction}: It addresses limitations in objective SE benchmarks by quantifying the user's subjective experience, specifically measuring how well a coding assistant explains complex concepts (\textit{Expression Naturalness}) and how effectively it adapts its technical solutions based on developer feedback (\textit{Feedback Adaptability}) \cite{Guo2025HumanCentricEvaluation}.
\end{itemize}
