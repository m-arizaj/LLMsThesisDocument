...
Planning metrics are essential for evaluating LLM-based agents in software engineering, focusing on efficiency, correctness, and reliability.

\begin{itemize}
    \item \textbf{Autonomous Agents Evaluation}: Metrics like \textbf{Length of Planning} assess the operational cost (time/tokens) of agents such as ChatDev or MetaGPT, while \textbf{Plan Score} validates adherence to development workflows (e.g., coding before testing) \cite{Wang2024AutonomousAgentsSurvey, Xu2025LLMAgentsToolLearning}.
    
    \item \textbf{Tool Chain Verification}: In coding assistants, metrics ensure correct tool sequencing (e.g., linter $\rightarrow$ compiler) and efficient error recovery strategies \cite{chen2023teval, Xu2025LLMAgentsToolLearning}.
    
    \item \textbf{Optimal System Control}: Frameworks like \textbf{LLM+P} utilize planning metrics to minimize steps in critical system operations, guaranteeing optimal execution via formal solvers \cite{Liu2023LLMP}.
\end{itemize}
